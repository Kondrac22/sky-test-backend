...
import { DeleteUserFavoriteMediaSchema, GetMediaByIdSchema, GetMediasSchema, GetUserFavoritesListSchema,  PostMediaSchema, PostSaveFavoriteSchema } from "../../schemas";
...
export async function routes(app: FastifyTypeInstance) {
    ...
    app.post('/users/:userId/favorites', PostSaveFavoriteSchema, async (request, reply) => {
        try {
            userHandler.saveFavoriteHandler(request.params.userId, request.body.mediaId)
    ...
    app.get('/users/:userId/favorites', GetUserFavoritesListSchema, async (request, reply) => {
        const userFavoriteList = userHandler.getUserFavoritesListHandler(request.params.userId)
        return reply.status(200).send(userFavoriteList)
    })

    app.delete('/users/:userId/favorites/:mediaId', DeleteUserFavoriteMediaSchema, async (request, reply) => {

        try {
            const deleteFavorite = userHandler.deleteMediaHandler(request.params.userId, request.params.mediaId)
            return reply.status(204).send()
        } catch {
